...
def random_tensors(shape, num=1, requires_grad=False):
    tensors = [torch.randn(shape, requires_grad=requires_grad)
...
class EinsumAttention():
    def __init__(self):
        ...
        self.WY, self.Wh, self.Wr, self.Wt = \
            random_tensors([in_hid, out_hid], num=4, requires_grad=True)
        self.bM, self.br, self.w = random_tensors([out_hid], num=3, requires_grad=True)

    def forward(self, Y, ht, rt1):
        # -- [batch_size x hidden_dimension]
        tmp = torch.einsum("ik,kl->il", [ht, self.Wh]) + \
              torch.einsum("ik,kl->il", [rt1, self.Wr])

        Mt = torch.tanh(torch.einsum("ijk,kl->ijl", [Y, self.WY]) + \
                    tmp.unsqueeze(1).expand_as(Y) + self.bM)
        # -- [batch_size x sequence_length]
        at = F.softmax(torch.einsum("ijk,k->ij", [Mt, self.w]), dim=-1)

        # -- [batch_size x hidden_dimension]
        rt = torch.einsum("ijk,ij->ik", [Y, at]) + torch.tanh(torch.einsum("ij,jk->ik", [rt1, self.Wt]) + self.br)
