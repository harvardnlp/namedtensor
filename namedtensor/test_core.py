...
def test_shift():
    for ntensor in make_tensors(dict(alpha=10, beta=2, gamma=50)):
        ...
        assert ntensor.named_shape == OrderedDict(
            [("beta", 2), ("gamma", 50), ("alpha", 10)]
        )
...
def test_unbind():
    ...
    assert a.named_shape == OrderedDict([("alpha", 10), ("gamma", 50)])
...
def test_takes():
    ...
    assert selected.named_shape == OrderedDict(
...
def test_narrow():
    ...
    assert narrowed.named_shape == OrderedDict(
